...
qplot(training$CompressiveStrength,1:nrow(training),color=training$FlyAsh)
...
ILtraining = training[,grep("^IL_.*$",names(training))]
a=preProcess(ILtraining,method = "pca",thresh = 0.90)
...
ILtraining = training[,c(1,grep("IL_.*$",names(training)))]
a = train(diagnosis~.,data=ILtraining,method="glm",
          trControl = trainControl(preProcOptions = list(thresh = 0.8)),preProcess="pca")
b = train(diagnosis~.,data=ILtraining,method="glm")
